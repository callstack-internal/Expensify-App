...
### [@rock-js+provider-s3+0.11.5+002+temporary-test-logs.patch](@rock-js+provider-s3+0.11.5+002+temporary-test-logs.patch)

- Reason:

    ```
    This patch adds temporary debug logging to help troubleshoot S3 provider authentication 
    and operations. It logs which authentication method is being used (access keys, role ARN, 
    profile, or public access) and adds debug logs for list, download, and upload operations 
    to track the flow and identify issues with S3 bucket access and credential resolution.
    ```
