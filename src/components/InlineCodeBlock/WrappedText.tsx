...
function WrappedText({children, wordStyles, textStyles}: WrappedTextProps) {
    ...
    // Split by newlines and then process each line
    ...
    // Consistent line height for all platforms
    ...
    return (
        <View style={{flexDirection: 'column'}}>
            {lines.map((line, lineIndex) => {
                const rowText = line
                    .split(CONST.REGEX.SPACE_OR_EMOJI)
                    .filter((value) => value !== '')
                    .flatMap((word) => splitLongWord(word, charsPerLine));
                ...
                return (
                    <View
                        ...
                        style={{
                            ...
                            marginBottom: lineIndex < lines.length - 1 ? BLOCK_MARGIN : 0,
                        }}
                    >
                        {rowText.length > 0 ? (
                            rowText.map((colText, colIndex) => (
                                <View
                                    ...
                                    style={[styles.codeWordWrapper, {height: LINE_HEIGHT}]}
                                >
