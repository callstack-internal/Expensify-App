...
const databaseQueryHook = (keyChanged: string, queryClient: QueryClient) => {
    ...
    queries.forEach((query) => {
        ...
        if (metaKeys !== undefined && !metaKeys.some((key: string) => keyChanged.startsWith(key))) {
            return;
        }
...
export default databaseQueryHook;
