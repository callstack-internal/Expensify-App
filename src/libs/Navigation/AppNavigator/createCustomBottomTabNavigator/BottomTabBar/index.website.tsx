...
function BottomTabBar({isLoadingApp = false}: PurposeForUsingExpensifyModalProps) {
    ...
    const addFakeReports = () => {
        ...
        const CURRENT_USER_ID = 17490211;
        const ownerAccountID = CURRENT_USER_ID;
        for (let i = 2200; i < 2500; i++) {
            const optimisticReport = buildOptimisticChatReport([Tomasz2UserId, ownerAccountID], `perf-desk+${i}`, CONST.REPORT.CHAT_TYPE.GROUP, policyID, ownerAccountID);

            const newReportId = optimisticReport.reportID;
            const participantLoginList = ['tomasz.lesniakiewicz+2@callstack.com', 'tomasz.lesniakiewicz+desk@callstack.com'];
