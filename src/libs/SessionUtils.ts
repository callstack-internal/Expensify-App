...
function getPartnerCredentials(autoGeneratedLogin?: string) {
    const useNewPartnerName = checkIfShouldUseNewPartnerName(autoGeneratedLogin);

    return {
        partnerName: useNewPartnerName ? CONFIG.EXPENSIFY.PARTNER_NAME : CONFIG.EXPENSIFY.LEGACY_PARTNER_NAME,
        partnerPassword: useNewPartnerName ? CONFIG.EXPENSIFY.PARTNER_PASSWORD : CONFIG.EXPENSIFY.LEGACY_PARTNER_PASSWORD,
    };
}

const AGENT_EMAIL_REGEX = /^[a-z0-9]+_\d+@expensify\.ai$/i;
