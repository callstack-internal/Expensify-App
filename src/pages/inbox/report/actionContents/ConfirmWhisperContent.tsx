...
function ConfirmWhisperContent({action, reportID, originalReportID, actionReportID}: ConfirmWhisperContentProps) {
    const isOriginalReportArchived = useReportIsArchived(originalReportID);
    const mentionReportContextValue = {currentReportID: reportID, exactlyMatch: true};

    // Subscribe to the full report here — the resolve action needs heartbeat fields for its
    // failure-revert payload that the stable projection strips.
