 ...
 *
 * Commented-out `last*` fields are deliberately omitted: they are "heartbeat" fields that
 * update on routine activity (incoming/outgoing messages, read receipts, etc.) and would
 * invalidate the memo on every chat heartbeat without any of the item subtree actually
 * reading them. Everything else stays in the projection so consumer utils that read
 * Report fields keep working without surprise `undefined`s.
 */
function stableReportSelector(report: OnyxEntry<Report>): OnyxEntry<Report> {
    if (!report?.reportID) {
        return undefined;
    }
    return {
...
export {getArchiveReason, getReportChatType, getReportOwnerAccountID, getReportPolicyID, openExpenseReportIDsSelector, stableReportSelector};
